...
class SelectFilesLegacy extends React.Component {
    constructor(props) {
        super(props);
        this.props = props;
        this.state = {
    ...
    componentDidMount() {
        ...
        this.FilePearl.subscribe((newState) => {
            this.setState({file: newState.legacyFile});
        });
    }
    ...
    changeFile(evt) {
        this.FilePearl.setFile(evt.target.files[0], 'legacy')
            .then(() => {
    ...
    render() {
        return (
            <div className="relative h-full w-full">
                <h2 className="font-normal text-grey-darkest">Select Files - Legacy File</h2>
                <p className="pt-4">
                    Please upload the legacy data file. Note that the type will be auto-detected, but must be of the
...
export default SelectFilesLegacy;